...
<?php
if ( ! isset( $fhai_currency_code ) ) {
	$fhai_currency_code = '';
}

if ( ! isset( $fhai_currency_symbol ) ) {
...
foreach ( $products as $fhai_product ) :
	...
	$fhai_background_color_raw   = $fhai_product['pricing-background-color'] ?? '';
	...
	// -------------------------------------------------
	if ( class_exists( 'NumberFormatter' ) && $fhai_currency_code ) {
		$fhai_formatter      = new NumberFormatter( 'en', NumberFormatter::CURRENCY );
		$fhai_display_price  = $fhai_formatter->formatCurrency( $fhai_effective_price, $fhai_currency_code );
		$fhai_original_price = $fhai_formatter->formatCurrency( $fhai_monthly_price, $fhai_currency_code );
	} elseif ( 'INR' === $fhai_currency_code ) {
			$fhai_display_price  = $fhai_currency_symbol . fhai_indian_number_format( $fhai_effective_price );
			$fhai_original_price = $fhai_currency_symbol . fhai_indian_number_format( $fhai_monthly_price );
	} else {
		$fhai_display_price  = $fhai_currency_symbol . number_format( $fhai_effective_price, 2 );
		$fhai_original_price = $fhai_currency_symbol . number_format( $fhai_monthly_price, 2 );
	}
	...
	<?php
		echo esc_attr(
			$atts['style'] .
			( 1 === (int) $fhai_product['highlight'] ? ' highlighted-product-container' : '' ) .
			$fhai_product_styles_group1 .
			$fhai_product_styles_group2 .
			$fhai_product_styles_group3 .
			$fhai_product_styles_group4 .
	...
	?>
		...
		data-currency-code="<?php echo esc_attr( $fhai_currency_code ); ?>"
		data-currency-symbol="<?php echo esc_attr( $fhai_currency_symbol ); ?>"
	...
	>
		...
		<div class="additional-container">
			...
			<?php endif; ?>>
				...
				<div class="product-pricing <?php echo 1 === (int) $fhai_product['highlight'] ? 'highlighted-product' : ''; ?>">

					<?php if ( 1 === (int) $fhai_product['highlight'] ) : ?>
